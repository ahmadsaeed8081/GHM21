      ...
      <div className="relative mx-auto flex h-[56px] w-full max-w-7xl items-center gap-2 sm:h-[68px]">
        ...
        <div className="flex min-w-0 items-center gap-2 max-sm:gap-1.5">
          ...
          {/* <div className="flex min-w-0 flex-col leading-tight">
            <span className="truncate text-[12px] font-bold tracking-[0.08em] text-white sm:text-[16px]">
              GH MARKETPLACE
            </span>
            <span className="truncate text-[9px] font-medium tracking-[0.12em] text-gray-400 sm:text-[14px] sm:text-gray-300">
              CODE IS LAW
            </span>
          </div> */}
        </div>

        {/* Nav pill — centered on desktop, vertical list on mobile */}
        <div
        ...
        >
          {/* Desktop pill */}
          <ul className="m-0 hidden list-none items-center gap-1 rounded-full bg-white/[0.06] border border-white/[0.10] p-1.5 nav:flex">
            {navLinks.map((link) => (
              <li key={link}>
                <a
                  href="#"
          ...
          <ul className="m-0 flex list-none flex-col nav:hidden">
            {navLinks.map((link) => (
              <li key={link} className="border-b border-white/[0.08] last:border-b-0">
                <a
                  href="#"
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2 max-sm:gap-1.5">
          ...
          >
            <img src={wallet} alt="" className="h-[18px] w-[18px] shrink-0 sm:h-5 sm:w-5" />
        ...
        {isConnected?(
          ...
          aria-label="Profile"
        >
          <img src={user} alt="" className="h-[18px] w-[18px] sm:h-5 sm:w-5" />
        </button>
